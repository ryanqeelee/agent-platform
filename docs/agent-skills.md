...
Sandbox 不再把凭据和模板放进 `WEKNORA_SANDBOX_*`。后端、凭据、模板、执行超时、TTL 和私网访问策略均由系统管理员在「设置 → 沙箱后端」作为平台配置保存；新会话没有显式绑定时使用平台默认连接。Docker 后端默认关闭，由系统管理员在「设置 → 系统设置」打开，或设置 `WEKNORA_SANDBOX_DOCKER_ENABLED=true`。

### Sandbox 模式

Docker、CubeSandbox、E2B 均通过同一套平台配置 CRUD、连接检查和智能体选择接口管理。CubeSandbox / E2B 的集群搭建和设置页接入流程见 [WeKnora 沙箱集群与标准模板](sandbox-cluster.md)。设置页会通过当前连接拉取模板目录；若没有 WeKnora 标准模板，后端会从标准镜像发起创建，用户无需复制模板 ID。
...
平台可以维护**多份具名**沙箱后端配置（「设置 → 沙箱后端」），智能体在编辑弹窗的「能力扩展 → 沙箱后端」里可显式选择一份；没有显式绑定的新会话使用平台默认连接。

同一后端类型可以有多份配置：例如两份 E2B 分别指向不同账号或区域，让不同智能体的技能脚本落在不同配额上。

**平台配置是唯一运行时来源。** 端点、凭据和运行参数不会从 `.env` 回退：
...
平台「技能管理」设置里可以把技能登记到共享目录，再装进一个或多个沙箱配置的镜像。除上传 zip 外，也支持从托管平台粘贴来源（每种写法只对应一种来源，不会猜测）：
