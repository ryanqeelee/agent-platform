...
test('focused skill management expands env vars and transcript without a mid-page save button', () => {
  ...
  assert.match(source, /hasInstallTimeline\(skill: ConfigSkill\)[\s\S]*?Boolean\(skill\.install_run_id\)/)
  assert.match(source, /:run-id="(?:managedSkill|skill)\.install_run_id \|\| ''"/)
  assert.match(source, /:message-id="(?:managedSkill|skill)\.id"/)
  assert.doesNotMatch(source, /install_session_id|install_message_id/)
