export default {
  enterpriseSkills: {
    ...
    "description": "Skills available to the employee assistant are configured centrally by platform administrators.",
    "empty": "No skills are currently available. Skill configuration is managed centrally by platform administrators.",
  ...
  agent: {
    ...
    editor: {
      ...
      automaticModel: 'Select model automatically',
      automaticModelHint: 'When unspecified, prefer the chat model configured by the knowledge base. If the knowledge base does not specify one, use the platform default or another available chat model.',
      automaticModelUnavailableHint: 'No chat model is currently available. You can save automatic selection, but at least one chat model must be configured and activated before this agent can run.',
      platformModelDesc: 'Choose a chat model for this platform agent, or keep automatic selection.',
      builtinModeFixed: 'Fixed mode: {mode}',
      employeeBuiltinModeHint: 'Employees can choose Quick Query or Deep Processing. Both use this configuration; Quick mode limits tool capabilities.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      description: 'Configure platform sandbox connections for isolated agent skill execution.',
      pageHintTitle: 'What is a sandbox?',
      pageHint: 'A sandbox is the isolated environment where agent skill scripts run. The platform can have several connections (Docker, E2B, CubeSandbox); the default serves new sessions unless an agent has an existing pinned connection. Skills are installed on the Skill Management page into a selected connection\'s image.',
      editorDescription: 'Configure a platform runtime. Docker, CubeSandbox, and E2B use the same management flow.',
      ...
      backendDescriptions: {
        ...
        docker: 'Keep a long-lived container per session on this Huanshu host; scripts and files stay in that container',
      },
      ...
      dockerHostRisk: 'Empty or unix:// uses the Docker daemon on the Huanshu host, which is equivalent to root on that machine. Use this only for a private single-node install. Prefer Cube or E2B when multiple workspaces share a host. Remote tcp:// endpoints require a TLS certificate directory.',
      addConfig: 'Add sandbox',
      setDefault: 'Set as default',
      defaultUpdated: 'Default sandbox updated',
      defaultUpdateFailed: 'Failed to update the default sandbox',
      ...
      noConfigs: 'No platform sandbox is configured, so agents cannot run skill scripts.',
      ...
      inventorySessions: 'Occupying session IDs',
      inventorySessionKind: 'Session ID',
      ...
      blockedHint: 'Ask the enterprise administrator to end or delete those sessions, or create a second config and point the agents at it.',
      ...
      namedBackendHint: 'The platform sandbox connection is the runtime source. New sessions use the default connection.',
      ...
      templateLoadHint: 'Enter the cluster connection and refresh. If there is no Huanshu standard template, click Create; after changing DNS or the image, click Delete and rebuild.',
      ...
      templateStepHint: 'This step only lists templates already on the cluster; it does not create one automatically. Click Create Huanshu standard template if none exists, or Delete and rebuild after changing DNS or the image. You can continue after a template is ready.',
      ...
      templateUntaggedHint: 'The builds finished but none carries the default tag, so sandbox creation cannot resolve this template. Delete it in E2B and refresh; Huanshu will rebuild it.',
      templateFailedReason: 'Build failed: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      weknoraStandardTemplate: 'Huanshu standard template',
      ...
      replaceStandardTemplateConfirm: 'Rebuild the Huanshu standard template with the current settings, including DNS. The previous spawnable template is not deleted until the replacement is ready.',
      ...
      envKey: 'Name',
      envValue: 'Value',
      envVarsHint: 'Injected into every sandbox using this platform connection. Values are encrypted at rest but visible to scripts running inside the sandbox.',
      addRow: 'Add variable',
      removeRow: 'Remove',
      save: 'Save',
      ...
      skillEnv: {
        ...
        platformTitle: 'Platform default values',
        platformHint: 'These values apply when the current enterprise and signed-in user have no private value. Members set private values under Settings → Sandbox secrets.',
        required: 'Required',
        isSet: 'Set',
        notSet: 'Not set',
        ...
        save: 'Save',
        saveSuccess: 'Platform default values saved',
        saveFailed: 'Could not save the platform default values.',
        clear: 'Clear',
        clearConfirm: 'Clear the platform default for {name}? The declaration stays. Users without a private value will then be missing it.',
        clearSuccess: 'Platform default value cleared.',
        valueTooLong: 'A value cannot exceed {max} bytes.'
      },
    },
    skills: {
      title: 'Skill Management',
      description: 'Skills live in the platform catalog. Register them first, then install onto one or more sandboxes. An agent can only enable skills that are ready on its sandbox.',
      ...
      addStepRegisterDesc: 'Paste a source or upload a zip to add the skill to the platform catalog.',
    ...
    storageBackend: {
      ...
      defaultTag: 'Default',
      defaultRequired: 'Set a platform default storage instance for new knowledge bases.',
  ...
  vectorStoreSettings: {
    ...
    defaultRequired: 'Set a platform default vector database for new knowledge bases.',
    defaultTag: 'Default',
    setDefault: 'Set as default',
    defaultUpdated: 'Default vector database updated',
    basicSection: 'Basic',
  ...
  envVarSettings: {
    ...
    description: 'Personal keys for skills and sandboxes, not Huanshu system or deployment settings.',
    ...
    noConfigDescription: 'No platform sandbox is configured yet, so there are no keys to configure.',
    ...
    skillHint: 'Each skill declares what it needs. Save private values here ahead of time, or supply them in the conversation. Platform admins can also set defaults on the skill card.',
    ...
    statusUnset: 'Not set',
    statusWorkspace: 'Using the platform default',
    ...
    clearConfirm: 'Remove your own value for {name}? The platform default, if there is one, applies again afterwards.',
  ...
  chatHistorySettings: {
    ...
    description: 'Configure one platform policy for semantic search over each tenant’s private conversation index',
    enableLabel: 'Enable Message Indexing',
    enableDescription: 'When enabled, new eligible messages are indexed. Disabling stops new index jobs; work already queued may finish.',
    embeddingModelLabel: 'Embedding Model',
    embeddingModelDescription: 'Select the embedding model for vectorizing chat messages',
    embeddingModelLocked: 'Tenant indexes exist; only their current model can be reused. Clear the internal indexes before selecting another model.',
  ...
  knowledgeList: {
    ...
    uninitializedEnterpriseBanner: 'Search is not ready for some knowledge bases. Contact your platform administrator to complete setup.',
  ...
  knowledgeEditor: {
    ...
    basic: {
      ...
      typeFAQ: 'FAQ Q&A',
      typeDescription: 'Use documents for policies, contracts and manuals; use Q&A for standard questions and answers.',
  ...
  kbSettings: {
    vectorStore: {
      ...
      engineDesc: 'Pick a vector store from the global configuration. New knowledge bases preselect the platform default.',
      boundLabel: 'Bound vector store',
      systemDefault: 'System default',
      selectPlaceholder: 'Select a vector store',
      defaultTag: 'Platform default',
      defaultRequired: 'No platform default vector store is configured. Configure one in Vector Store Settings first.',
    ...
    storage: {
      ...
      defaultTag: 'Default',
      defaultRequired: 'No platform default storage instance is configured. Configure one in Storage Settings first.',
  ...
  agentEditor: {
    ...
    desc: {
      ...
      maxTokensAgent: 'Maximum tokens generated in each reasoning round, including tool-call JSON. Default is 4096 without a sandbox, or 24576 when a sandbox can write or edit files. Custom values are saved as entered; with a sandbox, the runtime enforces a minimum of 8192.',
