export default {
  enterpriseSkills: {
    "title": "직원 도우미 스킬",
    "description": "직원 도우미가 사용할 수 있는 스킬은 플랫폼 관리자가 중앙에서 구성합니다.",
    "empty": "현재 사용할 수 있는 스킬이 없습니다. 스킬 구성은 플랫폼 관리자가 중앙에서 관리합니다.",
  ...
  agentEditor: {
    ...
    desc: {
      ...
      maxTokensAgent: '각 추론 라운드에서 생성할 최대 토큰 수(도구 호출 JSON 포함). 「기본값」은 샌드박스 없으면 4096, 파일 쓰기/편집이 가능하면 24576입니다. 사용자 지정 값은 입력한 대로 저장되며, 샌드박스가 있으면 런타임에서 최소 8192를 적용합니다.',
  ...
  kbSettings: {
    ...
    storage: {
      ...
      defaultTag: '기본',
      defaultRequired: '플랫폼 기본 스토리지 인스턴스가 설정되지 않았습니다. 먼저 스토리지 설정에서 구성하세요.',
    ...
    vectorStore: {
      ...
      engineLabel: '벡터 스토어',
      engineDesc: '전역 설정에서 벡터 스토어를 선택합니다. 새 지식베이스에는 플랫폼 기본값이 미리 선택됩니다.',
      boundLabel: '바인딩된 벡터 스토어',
      systemDefault: '시스템 기본값',
      selectPlaceholder: '벡터 스토리지 선택',
      defaultTag: '플랫폼 기본값',
      defaultRequired: '플랫폼 기본 벡터 스토리지가 설정되지 않았습니다. 먼저 벡터 스토리지 설정에서 구성하세요.',
  ...
  knowledgeEditor: {
    ...
    postCreateHint: {
      title: '생성 완료',
      footer: '문서를 업로드하기 전에 지식 접근 범위를 확인하세요. 필요하면 데이터 소스를 설정한 후 저장하고 닫으세요.',
    ...
    basic: {
      ...
      typeDocument: '문서',
      typeFAQ: 'Q&A',
      typeDescription: '문서는 정책, 계약 및 매뉴얼에 적합하고, Q&A는 표준 질문과 답변에 적합합니다.',
      nameLabel: '지식베이스 이름',
  ...
  knowledgeList: {
    ...
    uninitializedEnterpriseBanner: '일부 지식베이스의 검색 서비스가 준비되지 않았습니다. 플랫폼 관리자에게 설정을 요청하세요.',
  ...
  envVarSettings: {
    title: '샌드박스 키',
    description: '스킬과 샌드박스에 쓰는 개인 키이며, Huanshu 시스템이나 배포 설정이 아닙니다.',
    helpAria: '샌드박스 키 설명',
    ...
    noConfigDescription: '플랫폼 샌드박스 연결이 아직 없어 설정할 키가 없습니다.',
    ...
    skillHint: '각 스킬에 필요한 비공개 값을 여기에서 미리 저장하거나 대화 중 제공할 수 있습니다. 플랫폼 관리자는 스킬 카드에 공용 기본값을 설정할 수 있습니다.',
    ...
    requiredTag: '필수',
    statusUnset: '설정되지 않음',
    statusWorkspace: '플랫폼 공용 기본값 사용 중',
    statusUser: '내가 설정함',
    setValue: '설정',
    replaceValue: '변경',
    addRow: '추가',
    ...
    clear: '내 값 삭제',
    clearConfirm: '{name}에 설정한 내 값을 삭제할까요? 이후에는 플랫폼 공용 기본값이 있으면 다시 적용됩니다.',
  ...
  chatHistorySettings: {
    title: '메시지 관리',
    description: '모든 테넌트에 하나의 플랫폼 메시지 인덱싱 정책을 적용하며 대화 인덱스는 테넌트별로 분리됩니다',
    enableLabel: '메시지 인덱싱 활성화',
    enableDescription: '활성화하면 새 대상 메시지를 인덱싱합니다. 비활성화하면 새 작업은 중지되지만 이미 대기 중인 작업은 완료될 수 있습니다',
    embeddingModelLabel: 'Embedding 모델',
    embeddingModelDescription: '메시지 벡터화에 사용할 Embedding 모델을 선택하세요',
    embeddingModelLocked: '테넌트 인덱스가 있어 기존 모델만 다시 사용할 수 있습니다. 다른 Embedding 모델을 선택하려면 내부 인덱스를 먼저 비워야 합니다',
    statsTitle: '인덱스 통계',
  ...
  vectorStoreSettings: {
    ...
    defaultRequired: '새 지식 베이스에서 사용할 플랫폼 기본 벡터 데이터베이스를 설정하세요.',
    defaultTag: '기본',
    setDefault: '기본으로 설정',
    defaultUpdated: '기본 벡터 데이터베이스가 업데이트되었습니다',
    basicSection: '기본',
  ...
  settings: {
    ...
    sandbox: {
      title: '샌드박스 설정',
      description: '에이전트 스킬 실행을 위한 플랫폼 공용 샌드박스 연결을 구성합니다.',
      pageHintTitle: '샌드박스란?',
      pageHint: '샌드박스는 에이전트 스킬 스크립트가 실행되는 격리 환경입니다. 플랫폼에 Docker, E2B, CubeSandbox 연결을 여러 개 둘 수 있으며 새 세션은 기본 연결을 사용합니다. 기존 세션은 고정된 연결을 유지합니다.',
      editorDescription: '플랫폼 런타임을 구성합니다. Docker, CubeSandbox, E2B는 같은 관리 흐름을 사용합니다.',
      ...
      backendDescriptions: {
        ...
        docker: '이 Huanshu 호스트의 Docker에서 세션마다 장수명 컨테이너를 유지합니다. 스크립트와 파일이 같은 컨테이너에 남습니다',
      },
      ...
      dockerHostRisk: '비우거나 unix:// 를 쓰면 Huanshu가 있는 머신의 Docker 데몬을 사용하며, 그 권한은 해당 머신의 root와 같습니다. 프라이빗 단일 노드에만 쓰세요. 여러 워크스페이스가 같은 호스트를 쓰는 경우에는 Cube 또는 E2B를 쓰세요. 원격 tcp:// 는 TLS 인증서 디렉터리가 필요합니다.',
      addConfig: '샌드박스 추가',
      setDefault: '기본으로 설정',
      defaultUpdated: '기본 샌드박스가 업데이트되었습니다',
      defaultUpdateFailed: '기본 샌드박스를 업데이트하지 못했습니다',
      ...
      noConfigs: '플랫폼 샌드박스 연결이 없어 에이전트가 스킬 스크립트를 실행할 수 없습니다.',
      ...
      inventorySessions: '점유 중인 세션 ID',
      inventorySessionKind: '세션 ID',
      ...
      blockedHint: '기업 관리자에게 해당 세션을 종료하거나 삭제하도록 요청하거나, 새 설정을 만들어 에이전트가 사용하도록 지정하세요.',
      ...
      namedBackendHint: '플랫폼 샌드박스 연결이 런타임 소스이며 새 세션은 기본 연결을 사용합니다.',
      ...
      templateLoadHint: '클러스터 연결을 입력한 뒤 새로고침하세요. Huanshu 표준 템플릿이 없으면 「만들기」를, DNS나 이미지를 바꾼 뒤에는 「삭제 후 재구축」을 누르세요.',
      ...
      templateStepHint: '이 단계에서는 클러스터에 이미 있는 템플릿만 나열하며 자동으로 만들지 않습니다. 표준 템플릿이 없으면 「Huanshu 표준 템플릿 만들기」를, DNS나 이미지를 바꾼 뒤에는 「삭제 후 재구축」을 누르세요. 템플릿이 준비된 뒤에 계속할 수 있습니다.',
      loadingTemplates: 'Loading templates from the cluster...',
      templateBuildingHint: '표준 템플릿을 구축하는 중입니다. 목록이 자동으로 새로고침됩니다.',
      templateUntaggedHint: '빌드는 끝났지만 default 태그가 붙은 빌드가 없어 샌드박스 생성 시 이 템플릿을 찾을 수 없습니다. E2B에서 템플릿을 삭제하고 새로고침하면 Huanshu가 다시 빌드합니다.',
      templateFailedReason: '빌드 실패: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      weknoraStandardTemplate: 'Huanshu 표준 템플릿',
      ...
      replaceStandardTemplateConfirm: '현재 설정(DNS 포함)으로 Huanshu 표준 템플릿을 다시 만듭니다. 새 템플릿이 준비되기 전에는 기존에 쓸 수 있는 템플릿을 삭제하지 않습니다.',
      ...
      cubeDnsServersHelp: '선택 사항입니다. Huanshu 표준 템플릿에 기록되는 nameserver IP입니다. 비워 두면 클러스터 기본값(보통 119.29.29.29)을 사용합니다. 공용 DNS로 UDP/53이 나가지 않으면 Cube 호스트 /etc/resolv.conf에서 도달 가능한 주소를 넣고, 10/8, 172.16/12, 192.168/16은 피하세요. 이미 있는 표준 템플릿은 카드에서 「재구축」을 눌러야 변경이 적용됩니다.',
      ...
      envKey: 'Name',
      envValue: 'Value',
      envVarsHint: '이 플랫폼 연결을 사용하는 모든 샌드박스에 주입됩니다. 값은 암호화되어 저장되지만 샌드박스 내부의 스크립트에는 보입니다.',
      addRow: 'Add variable',
      removeRow: 'Remove',
      save: 'Save',
      ...
      skillEnv: {
        ...
        platformTitle: '플랫폼 공용 기본값',
        platformHint: '현재 엔터프라이즈와 로그인 사용자에게 비공개 값이 없을 때 이 기본값을 사용합니다. 구성원은 설정의 「샌드박스 키」에서 비공개 값을 설정할 수 있습니다.',
        required: '필수',
        isSet: '설정됨',
        notSet: '설정되지 않음',
        ...
        save: '저장',
        saveSuccess: '플랫폼 공용 기본값을 저장했습니다',
        saveFailed: '플랫폼 공용 기본값을 저장하지 못했습니다.',
        clear: '삭제',
        clearConfirm: '{name}의 플랫폼 공용 기본값을 삭제할까요? 선언은 유지되며 비공개 값이 없는 사용자는 이 값을 사용할 수 없습니다.',
        clearSuccess: '플랫폼 공용 기본값을 삭제했습니다.',
        valueTooLong: '값 하나는 {max}바이트를 초과할 수 없습니다.'
      },
    },
    skills: {
      title: '스킬 관리',
      description: '스킬은 플랫폼 카탈로그에 속합니다. 먼저 등록한 뒤 하나 이상의 샌드박스에 설치할 수 있습니다. 에이전트는 현재 샌드박스에서 준비된 스킬만 사용할 수 있습니다.',
      ...
      addStepRegister: '등록',
      addStepInstall: '설치',
      addStepRegisterDesc: '소스를 붙여넣거나 zip을 업로드해 먼저 플랫폼 카탈로그에 등록합니다.',
    ...
    storageBackend: {
      ...
      defaultTag: '기본',
      defaultRequired: '새 지식 베이스에서 사용할 플랫폼 기본 스토리지 인스턴스를 설정하세요.',
      add: '스토리지 인스턴스 추가',
  ...
  agent: {
    ...
    editor: {
      ...
      automaticModel: '모델 자동 선택',
      automaticModelHint: '지정하지 않으면 지식 베이스에 구성된 대화 모델을 우선 사용합니다. 지식 베이스에 모델이 지정되지 않았으면 플랫폼 기본 또는 다른 사용 가능한 대화 모델을 사용합니다.',
      automaticModelUnavailableHint: '현재 사용할 수 있는 대화 모델이 없습니다. 자동 선택 상태로 저장할 수 있지만, 실행하기 전에 최소 하나의 대화 모델을 구성하고 활성화해야 합니다.',
      platformModelDesc: '이 플랫폼 에이전트의 대화 모델을 선택하거나 자동 선택을 유지합니다.',
      builtinModeFixed: '고정 모드: {mode}',
      employeeBuiltinModeHint: '직원은 빠른 조회 또는 심층 처리를 선택할 수 있으며, 둘 다 이 구성을 사용합니다. 빠른 모드에서는 도구 기능이 제한됩니다.',
  ...
  newUserGuide: {
    ...
    steps: {
        ...
        title: '준비 완료',
        desc: '직원 어시스턴트, 기업 지식, 경영 분석의 핵심을 확인했습니다. 이름 옆 도움말 버튼에서 언제든 가이드를 다시 볼 수 있습니다.'
      },
      models: {
        title: '모델 구성하기',
      ...
      analysis: {
        title: '경영 분석 보기',
        desc: '관리된 기업 데이터를 바탕으로 지표와 이상 징후를 확인하고 근거가 포함된 경영 인사이트를 얻으세요.'
      },
      welcome: {
        title: 'Huanshu에 오신 것을 환영합니다',
        desc: '몇 단계만으로 직원 어시스턴트, 기업 지식, 경영 분석을 안내합니다. \'다음\'을 눌러 시작하세요.'
      }
    }
  },
  chatHeader: {
