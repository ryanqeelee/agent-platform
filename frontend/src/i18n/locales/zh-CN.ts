export default {
  enterpriseSkills: {
    "title": "员工助理技能",
    "description": "员工助理可用技能由平台管理员统一配置。",
    "empty": "当前没有可用技能。技能配置由平台管理员统一管理。",
  ...
  tenantMember: {
    create: { button: '创建员工', submit: '创建', name: '用户名', password: '初始密码', hint: '密码需为 8–32 位，包含字母和数字。创建后员工可直接登录。', nameRequired: '请输入用户名', passwordRequired: '请输入初始密码', success: '员工已创建', failed: '创建员工失败' },
    title: '成员管理',
    sectionDescription: '创建员工、分配岗位与权限，或停用和恢复账号。',
    listTitle: '企业成员',
    loading: '正在加载成员…',
    retry: '重试',
    empty: '暂无员工，点击创建员工。',
    ...
    errors: {
      ...
      lastOwner: '企业至少需要一名有效管理员，请先指定其他管理员。',
      notFound: '未找到对应的成员。',
      invalidRole: '角色必须是企业管理员或员工。',
      generic: '操作失败，请稍后重试。'
    },
    roleChange: {
      success: '角色已更新'
  ...
  integrations: {
    ...
    claw: {
      title: '环枢 Skill',
      subtitle: '通过环枢REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      installCtaHint: '安装环枢Skill · 将在新标签页打开',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合环枢自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用环枢桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    api: {
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在环枢与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与环枢保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
  ...
  agentEditor: {
    ...
    desc: {
      ...
      maxTokensAgent: '每一轮推理的最大生成 Token（含工具调用 JSON）。选「默认」时，未绑沙箱为 4096，绑了沙箱（可写/改文件）为 24576。自定义值会按填写保存；绑定沙箱时，运行时最低按 8192 执行。',
  ...
  kbSettings: {
    ...
    storage: {
      ...
      defaultTag: '默认',
      defaultRequired: '平台尚未设置默认存储实例，请先前往存储设置。',
      instanceLabel: '存储实例',
    ...
    vectorStore: {
      ...
      loading: '正在加载向量存储列表...',
      engineLabel: '向量存储',
      engineDesc: '从全局向量存储配置中选择。新建知识库会预选平台默认实例。',
      boundLabel: '已绑定的向量存储',
      systemDefault: '系统默认',
      selectPlaceholder: '选择向量存储',
      defaultTag: '平台默认',
      defaultRequired: '平台尚未设置默认向量存储，请先前往向量存储设置。',
  ...
  organization: {
    ...
    rbac: {
      needTenantAdminTip: '此操作需要当前空间的 admin 或更高角色，请联系企业管理员调整权限。',
  ...
  modelSettings: {
    ...
    runtimeContext: {
      ...
      retrievalTitle: '检索与文档处理方案', embedding: '向量化能力引用', reranking: '重排能力引用', parsing: '解析能力引用',
      unavailable: '当前无法读取能力方案上下文；模型配置仍保持平台专属。',
      retrievalUnavailable: '当前无法读取能力方案上下文；检索与处理配置仍保持平台专属。'
    },
    copySuffix: ' 副本',
    builtinTag: '内置',
  ...
  system: {
    ...
    globalSettings: {
      ...
      keyDescriptions: {
        ...
        auth: {
          registration_mode: '自助注册模式。self_serve = 任何人可注册账号；invite_only = 关闭公网注册，仅企业管理员可邀请。修改后立即生效，但谨慎对待 self_serve（公网会接受 spam）。',
          default_tenant_mode: '公开注册后的空间初始化策略。create_personal 会自动创建个人空间并授予企业管理员权限；tenantless 仅创建账户，用户需要接受邀请或主动创建空间。只影响之后注册的用户。',
  ...
  tenant: {
    ...
    currentTenant: '当前空间',
    switchTenant: '切换空间',
    sectionDescription: '查看和维护企业基本信息',
    statusActive: '活跃',
    statusInactive: '未激活',
    statusSuspended: '已暂停',
    statusUnknown: '未知',
    ...
    unknown: '未知',
    formatError: '格式错误',
    searchPlaceholder: '搜索企业名称或输入企业编号...',
    noMatch: '未找到匹配的空间',
    ...
    deleteDangerZone: {
      ...
      confirmHint: '请输入企业名称「{name}」以确认删除。',
      confirm: '确认删除',
      nameMismatch: '企业名称不匹配',
      success: '空间已删除',
      failed: '删除空间失败'
    },
    leaveDangerZone: {
    ...
    details: {
      idLabel: '企业编号',
      idDescription: '企业的唯一编号',
      nameLabel: '企业名称',
      nameDescription: '企业对外显示的名称',
      descriptionLabel: '企业简介',
      descriptionDescription: '简要介绍企业',
      businessLabel: '空间业务',
      businessDescription: '空间所属的业务类型',
      statusLabel: '企业状态',
      statusDescription: '企业当前的服务状态',
      createdAtLabel: '开通时间',
      createdAtDescription: '企业开通服务的时间',
      editName: '修改名称',
      editNamePlaceholder: '请输入新的企业名称',
      editNameConfirm: '保存',
      editNameCancel: '取消',
      editNameRequired: '企业名称不能为空',
      editNameSuccess: '企业名称已更新',
      editNameFailed: '更新企业名称失败',
      editDescription: '修改描述',
      editDescriptionPlaceholder: '请输入新的企业简介',
      editDescriptionSuccess: '企业简介已更新',
      editDescriptionFailed: '更新企业简介失败',
  ...
  knowledgeEditor: {
    ...
    postCreateHint: {
      title: '创建成功',
      footer: '上传资料前，请先确认知识访问范围；可继续配置数据源，完成后保存并关闭。',
    ...
    basic: {
      ...
      typeDocument: '文档',
      typeFAQ: '问答',
      typeDescription: '文档适合制度、合同与操作手册；问答适合维护标准问题和答案。',
      nameLabel: '知识库名称',
  ...
  knowledgeList: {
    ...
    uninitializedEnterpriseBanner: '部分知识库的检索服务尚未就绪，请联系平台管理员完成配置。',
    ...
    empty: {
      title: '暂无知识库',
      description: '点击“新建知识库”，添加企业制度、流程与业务资料',
      sharedTitle: '暂无共享知识库',
  ...
  envVarSettings: {
    title: '沙箱密钥',
    description: '给技能和沙箱用的个人密钥，不是环枢的系统或部署配置。',
    helpAria: '沙箱密钥说明',
    ...
    retry: '重试',
    noConfigTitle: '还没有沙箱',
    noConfigDescription: '平台还没有配置沙箱连接，暂时没有可设置的密钥。',
    sandboxTitle: '某个沙箱里始终带上的值',
    sandboxHint: '只带给你在那个沙箱里跑的命令。多数情况用不到；需要时再加，对话里也可以当场提供。',
    sandboxEmpty: '还没有添加过。',
    sandboxPick: '用在哪个沙箱',
    skillTitle: '技能要用的密钥',
    skillHint: '技能声明需要的值。可以预先在这里保存私有值，也可以在对话里当场提供。平台管理员还可以在技能卡上填写通用默认值。',
    ...
    deleteSuccess: '变量已删除。',
    clear: '清除我的值',
    clearConfirm: '清除你为 {name} 设置的值？之后会重新使用平台通用默认值（如果有）。',
    clearSuccess: '已清除你的值。',
  ...
  chatHistorySettings: {
    title: '消息管理',
    description: '统一配置平台消息索引策略，各企业的对话索引数据仍相互隔离',
    enableLabel: '启用消息索引',
    enableDescription: '开启后为新的合格消息创建索引；关闭后不再创建新任务，已经排队的任务可能继续完成',
    embeddingModelLabel: 'Embedding 模型',
    embeddingModelDescription: '选择用于消息向量化的 Embedding 模型',
    embeddingModelLocked: '已有企业消息索引，只能继续使用原模型；选择其他 Embedding 模型前需先清空内部索引',
    statsTitle: '索引统计',
  ...
  vectorStoreSettings: {
    ...
    defaultRequired: '请设置一个平台默认向量数据库，供新知识库使用。',
    defaultTag: '默认',
    setDefault: '设为默认',
    defaultUpdated: '默认向量数据库已更新',
    basicSection: '基本信息',
    storesTitle: '向量数据库',
    addStore: '添加数据库',
    editStore: '编辑数据库',
  ...
  settings: {
    ...
    sandbox: {
      title: '沙箱配置',
      description: '配置平台共享的隔离沙箱连接，用于执行智能体技能脚本。',
      pageHintTitle: '什么是沙箱？',
      pageHint: '沙箱是智能体执行技能脚本的隔离环境。平台可以添加多个 Docker、E2B 或 CubeSandbox 连接；新会话使用默认连接，已有会话保留原有绑定。技能在「技能管理」页安装到选定连接的镜像中。',
      editorDescription: '配置平台运行环境，Docker、CubeSandbox 和 E2B 使用同一套管理流程。',
      stepConnection: '连接',
      stepTemplate: '模板',
      stepRuntime: '运行配置',
      stepSkills: '技能',
      ...
      dockerHostRisk: '留空或 unix:// 会使用环枢所在机器的 Docker 守护进程，权限等同该机 root，只适合私有化单机。多套空间共用同一主机时请改用 Cube 或 E2B。远程 tcp:// 必须填写 TLS 证书目录。',
      addConfig: '添加沙箱',
      setDefault: '设为默认',
      defaultUpdated: '默认沙箱已更新',
      defaultUpdateFailed: '更新默认沙箱失败',
      ...
      addHeader: '添加 Header',
      removeRule: '删除该规则',
      noConfigs: '平台还没有沙箱连接，智能体无法执行技能脚本。',
      ...
      forceDelete: '强制删除',
      legacyConfig: '已废弃',
      namedBackendHint: '平台沙箱连接是运行时来源，新会话使用默认连接。',
      weknoraTemplateTitle: '环枢 标准模板',
      weknoraDockerImage: '环枢 标准镜像',
      weknoraDockerImageHint: '每次会话独占一个长驻容器，脚本、shell 命令与文件都在同一个容器内，会话结束或空闲超时后回收。',
      weknoraTemplateOverview: '环枢 负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含环枢技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      templateLoadHint: '填写集群连接后刷新模板。没有环枢标准模板时点「创建」；改 DNS 或镜像后点「删除并重建」。',
      templateLoadFailed: '模板列表加载失败',
      standardTemplateProvisioning: '已开始在集群中创建环枢标准模板，请稍后刷新查看状态',
      ...
      templateStepHint: '进入此步只会列出集群已有模板，不会自动创建。没有标准模板时点「创建环枢标准模板」；改 DNS 或镜像后点「删除并重建」。模板就绪后才能继续。',
      loadingTemplates: '正在从集群加载模板…',
      templateBuildingHint: '标准模板正在构建，列表会自动刷新。',
      templateUntaggedHint: '构建已完成，但没有构建带 default 标签，创建沙箱时无法解析。请在 E2B 删除该模板，刷新后环枢会重新构建。',
      templateFailedReason: '构建失败：{reason}',
      noTemplates: '当前集群未返回可用模板。',
      weknoraStandardTemplate: '环枢 标准模板',
      createStandardTemplate: '创建',
      createStandardTemplateHint: '按当前连接配置构建，包含 DNS。之后改配置可在卡片上重建。',
      replaceStandardTemplate: '重建',
      replaceStandardTemplateConfirm: '将用当前配置（含 DNS）重建环枢标准模板。新模板就绪前不会删除仍可启动的旧模板。',
      ...
      cubeDnsServersHelp: '可选。写入环枢标准模板的 nameserver（须为 IP）。留空则使用集群默认（常见 119.29.29.29）。私网或云上 UDP 53 出不去时，填 Cube 宿主机 /etc/resolv.conf 里能用的地址，并避开 10/8、172.16/12、192.168/16。已有标准模板需在模板卡片上点「重建」才会生效。',
      ...
      dockerTlsCertPathHelp: '环枢 所在主机上包含 ca.pem、cert.pem、key.pem 的目录。远程守护进程必填，证书不入库，由部署方挂载。',
      dockerIdleTtl: '空闲回收（秒）',
      dockerIdleTtlHelp: 'Docker 守护进程本身没有空闲超时。容器多久没有执行任何命令就会被环枢回收，会话继续时重建。留空按 1800 秒。',
      ...
      envKey: '变量名',
      envValue: '变量值',
      envVarsHint: '注入到使用此平台连接的每个沙箱；值加密存储，但对沙箱内脚本可见，请勿放入脚本不应读取的密钥。',
      addRow: '添加变量',
      removeRow: '删除',
      save: '保存',
      ...
      skillEnv: {
        ...
        toggleHide: '收起环境变量',
        none: '这个技能没有声明环境变量。',
        platformTitle: '平台通用默认值',
        platformHint: '当前企业和登录身份没有私有值时使用这里的默认值。成员可以在「设置 → 沙箱密钥」里填写私有值。',
        required: '必填',
        isSet: '已设置',
        notSet: '未设置',
        ...
        save: '保存',
        saveSuccess: '平台通用默认值已保存',
        saveFailed: '平台通用默认值保存失败。',
        clear: '清除',
        clearConfirm: '清除 {name} 的平台通用默认值？声明会保留，没有私有值的登录身份之后会缺少这个值。',
        clearSuccess: '已清除平台通用默认值。',
        valueTooLong: '单个值不能超过 {max} 字节。'
      },
    },
    skills: {
      title: '技能管理',
      description: '技能属于平台目录，可以只登记，也可以装到一份或多份沙箱。智能体只能启用当前沙箱里已就绪的技能。',
      ...
      addStepRegister: '登记',
      addStepInstall: '安装',
      addStepRegisterDesc: '粘贴来源或上传 zip，先登记到平台目录。',
    ...
    storageBackend: {
      ...
      empty: '尚未配置存储实例',
      defaultTag: '默认',
      defaultRequired: '请设置一个平台默认存储实例，供新知识库使用。',
      add: '添加存储实例',
      editTitle: '编辑存储实例',
      createTitle: '添加存储实例',
  ...
  agent: {
    ...
    editor: {
      ...
      automaticModel: '自动选择模型',
      automaticModelHint: '未指定时优先使用知识库配置的对话模型；知识库未指定模型时，使用平台默认或可用对话模型。',
      automaticModelUnavailableHint: '当前没有可用对话模型。可保存自动选择，但运行前需先配置并启用至少一个对话模型。',
      platformModelDesc: '为平台智能体指定对话模型，或保持自动选择。',
      builtinModeFixed: '固定模式：{mode}',
      employeeBuiltinModeHint: '员工端可选择快速查询或深入处理，共用此配置；快速模式会限制工具能力。',
      createTitle: '创建智能体',
      editTitle: '编辑智能体',
      buttons: {
        create: '创建智能体',
  ...
  newUserGuide: {
    ...
    steps: {
        ...
        title: '一切就绪',
        desc: '你已经了解员工助理、企业知识与经营分析。随时可点击菜单顶部昵称旁的帮助按钮重新查看引导。'
      },
      models: {
        title: '配置你的模型',
      ...
      analysis: {
        title: '查看经营分析',
        desc: '基于企业已治理的经营数据查看指标、定位异常，获得带依据的分析与洞察。'
      },
      welcome: {
        title: '欢迎使用环枢',
        desc: '只需几步，带你快速了解员工助理、企业知识与经营分析。点击「下一步」开始。'
      }
    }
  },
  chatHeader: {
