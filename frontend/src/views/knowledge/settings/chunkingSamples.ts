...
const MARKDOWN_SAMPLE = `# 环枢企业知识

环枢企业知识支持 RAG 问答、智能推理与 Wiki 知识组织。本文介绍其设计动机、架构与典型用法。

## 设计动机

企业内部知识散落在 Confluence、飞书、Notion、Git 仓库等多种系统，传统全文检索难以理解语义，单一 LLM 又缺少可信赖的上下文来源。平台的目标是：
...
\`\`\`bash
git clone https://github.com/ryanqeelee/agent-platform && cd agent-platform
...
const FAQ_SAMPLE = `# 环枢企业知识部署与使用 FAQ
...
\`\`\`
docker image inspect agent-server:local
docker image inspect agent-web:local
\`\`\`
