...
const troubleshootingDocsURL =
  'https://github.com/ryanqeelee/agent-platform/blob/agent-v1/docs/migration-troubleshooting.md'
...
const reportIssueURL = computed(() => {
  const base = 'https://github.com/ryanqeelee/agent-platform/issues/new'
  ...
  if (errMsg) {
    const body = [
      '### Environment',
      `- Agent platform version: ${systemInfo.value?.version || 'unknown'}`,
