		...
		for j, tc := range step.ToolCalls {
			...
			if !result.Success {
				...
				if isSandboxContentTool(tc.Name) {
					...
					// after the same bounded compaction used for successful commands.
					// External network tools remain on the opaque operational-status
					// projection because provider errors may carry credentials or URLs.
...
func CompactToolOutputForHistory(toolName string, result *types.ToolResult) string {
	...
	if isSandboxContentTool(toolName) {
		if rebuilt := compactSandboxHistory(result); rebuilt != "" {
			return compactHistoricalSandboxOutput(failedToolVisibleContent(rebuilt, result.Error))
		}
	}
	if !result.Success {
