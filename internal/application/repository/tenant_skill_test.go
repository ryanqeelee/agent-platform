...
func newSkillTestRepoWithDB(t *testing.T) (TenantSkillRepository, *gorm.DB) {
	...
	require.NoError(t, db.Exec(
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_platform_skills_config_name
		 ON platform_skills (sandbox_config_id, name) WHERE deleted_at IS NULL`).Error)
...
func skillRow(id, configID, name string) *types.TenantSkillEntity {
	return &types.TenantSkillEntity{
		ID: id, SandboxConfigID: configID, Name: name,
...
func TestSkillRepoIsolatesConfigs(t *testing.T) {
	...
	list, err := repo.ListSkillsByConfig(ctx, "cfg-1")
	require.NoError(t, err)
	require.Len(t, list, 1)
	require.Equal(t, "sk-a", list[0].ID)

	got, err := repo.GetSkill(ctx, "cfg-1", "sk-b")
	require.NoError(t, err)
	require.Nil(t, got, "a skill from another config must read as absent, not as an error")
}

func TestSkillRepoUpdatePersistsLifecycleWithoutOverwritingAdminOrRunState(t *testing.T) {
	repo := newSkillTestRepo(t)
	ctx := context.Background()
	require.NoError(t, repo.CreateSkill(ctx, skillRow("sk-a", "cfg-1", "pdf")))

	row, err := repo.GetSkill(ctx, "cfg-1", "sk-a")
	require.NoError(t, err)
	row.Status = types.SkillStatusReady
	row.InstalledSnapshotID = "snap-1"
	row.Enabled = false
	row.InstallRunID = "run-1"
	require.NoError(t, repo.UpdateSkill(ctx, row))

	got, err := repo.GetSkill(ctx, "cfg-1", "sk-a")
	require.NoError(t, err)
	require.Equal(t, types.SkillStatusReady, got.Status)
	require.Equal(t, "snap-1", got.InstalledSnapshotID)
	require.True(t, got.Enabled, "lifecycle writes must not overwrite the admin visibility switch")
	require.Empty(t, got.InstallRunID, "only BeginSkillRun may replace the operation token")
}

func TestSQLiteSkillMigrationMakesEmptyRunTokenComparable(t *testing.T) {
	db, err := gorm.Open(sqlite.Open("file:"+uuid.NewString()+"?mode=memory&cache=shared"), &gorm.Config{})
	require.NoError(t, err)
	require.NoError(t, db.Exec(migrationSQL(t, "migrations/sqlite/000034_platform_skills.up.sql")).Error)
	repo := NewTenantSkillRepository(db)
	ctx := context.Background()
	now := time.Now()
	require.NoError(t, db.Exec(`INSERT INTO platform_skills
		(id, sandbox_config_id, name, enabled, status, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?)`,
		"sk-ready", "cfg-1", "pdf", true, types.SkillStatusReady, now, now,
	).Error)

	row, err := repo.GetSkill(ctx, "cfg-1", "sk-ready")
	require.NoError(t, err)
	require.Empty(t, row.InstallRunID)
	row.Description = "empty-token write"
	matched, err := repo.UpdateSkillForRun(ctx, row, "")
	require.NoError(t, err)
	require.True(t, matched)

	require.NoError(t, repo.BeginSkillRun(
		ctx, "cfg-1", "sk-ready", "run-current", types.SkillStatusInstalling, now,
	))
	row.Description = "stale empty-token write"
	matched, err = repo.UpdateSkillForRun(ctx, row, "")
	require.NoError(t, err)
	require.False(t, matched)
	current, err := repo.GetSkill(ctx, "cfg-1", "sk-ready")
	require.NoError(t, err)
	require.NotEqual(t, "stale empty-token write", current.Description)
...
func TestSkillRepoInstallTranscriptRejectsAStaleRunToken(t *testing.T) {
	repo := newSkillTestRepo(t)
	ctx := context.Background()
	require.NoError(t, repo.CreateSkill(ctx, skillRow("sk-a", "cfg-1", "pdf")))
	started := time.Now()
	require.NoError(t, repo.BeginSkillRun(
		ctx, "cfg-1", "sk-a", "run-new", types.SkillStatusInstalling, started,
	))

	matched, err := repo.UpdateInstallTranscript(
		ctx, "cfg-1", "sk-a", "run-old", types.JSON(`[{"content":"stale"}]`),
	)
	require.NoError(t, err)
	require.False(t, matched)

	matched, err = repo.UpdateInstallTranscript(
		ctx, "cfg-1", "sk-a", "run-new", types.JSON(`[{"content":"current"}]`),
	)
	require.NoError(t, err)
	require.True(t, matched)
	got, err := repo.GetSkill(ctx, "cfg-1", "sk-a")
	require.NoError(t, err)
	require.JSONEq(t, `[{"content":"current"}]`, string(got.InstallTranscript))

	require.NoError(t, repo.BeginSkillRun(
		ctx, "cfg-1", "sk-a", "run-remove", types.SkillStatusRemoving, time.Now(),
	))
	got, err = repo.GetSkill(ctx, "cfg-1", "sk-a")
	require.NoError(t, err)
	require.Equal(t, "run-remove", got.InstallRunID)
	require.JSONEq(t, `[{"content":"current"}]`, string(got.InstallTranscript),
		"a removal attempt must retain the latest install history")
	matched, err = repo.UpdateInstallTranscript(
		ctx, "cfg-1", "sk-a", "run-new", types.JSON(`[{"content":"late"}]`),
	)
	require.NoError(t, err)
	require.False(t, matched, "a completed install cannot write through a newer removal token")
}
...
func TestSnapshotLedgerRecordsChain(t *testing.T) {
	...
	require.NoError(t, repo.CreateSnapshotRow(ctx, &types.TenantSkillSnapshotEntity{
		ID: "ins-1", SandboxConfigID: "cfg-1", SkillID: "sk-a",
	...
	rows, err := repo.ListSnapshotsByConfig(ctx, "cfg-1")
	require.NoError(t, err)
...
func TestSkillRepoSoftDeleteAllowsNameReuse(t *testing.T) {
	...
	ctx := context.Background()
	require.NoError(t, repo.CreateSkill(ctx, skillRow("sk-a", "cfg-1", "pdf")))
	require.NoError(t, repo.DeleteSkill(ctx, "cfg-1", "sk-a"))
	...
	got, err := repo.GetSkillByName(ctx, "cfg-1", "pdf")
	require.NoError(t, err)
	require.Equal(t, "sk-b", got.ID)
}

func TestMarkSnapshotStateUpdatesTheGlobalLedgerRow(t *testing.T) {
	repo := newSkillTestRepo(t)
	ctx := context.Background()
	require.NoError(t, repo.CreateSnapshotRow(ctx, &types.TenantSkillSnapshotEntity{
		ID: "ins-1", SandboxConfigID: "cfg-1", SkillID: "sk-a",
...
func TestSkillRepoUpdateSkillEnvsPersistsTheDeclarationEncrypted(t *testing.T) {
	...
	require.NoError(t, repo.UpdateSkillEnvs(ctx, "cfg-1", "sk-a", types.SkillEnvVars{
	...
	var stored string
	require.NoError(t, db.Raw(`SELECT envs FROM platform_skills WHERE id = ?`, "sk-a").Scan(&stored).Error)
...
func TestSkillRepoUpdateSkillLeavesEnvsAlone(t *testing.T) {
	...
	stale, err := repo.GetSkill(ctx, "cfg-1", "sk-a")
	require.NoError(t, err)
	require.NoError(t, repo.UpdateSkillEnvs(ctx, "cfg-1", "sk-a", types.SkillEnvVars{
...
func TestSkillRepoUpdateSkillAdminStateWritesBothColumns(t *testing.T) {
	...
	require.NoError(t, repo.UpdateSkillAdminState(ctx, "cfg-1", "sk-a", false,
...
func TestDeleteSkillPreservesTenantOwnedUserEnvs(t *testing.T) {
	t.Setenv("SYSTEM_AES_KEY", skillEnvTestAESKey)
	repo := newSkillTestRepo(t)
	ctx := context.Background()
	alice := types.Principal{Type: types.PrincipalWebUser, ID: "u-alice"}
	...
	keptA, err := repo.ListUserEnvVars(ctx, 7, alice, "cfg-1", "sk-a")
	require.NoError(t, err)
	require.Len(t, keptA, 1, "platform deletion must not delete enterprise private values")
...
func TestDeleteSkillWithMismatchedConfigKeepsSkillAndUserEnvs(t *testing.T) {
	...
	require.NoError(t, repo.DeleteSkill(ctx, "cfg-stale", "sk-a"),
		"a missing scoped row remains an idempotent no-op")

	skill, err := repo.GetSkill(ctx, "cfg-1", "sk-a")
	require.NoError(t, err)
...
func TestCountUserEnvVarsByConfigSpansEveryTenantAndPrincipal(t *testing.T) {
	t.Setenv("SYSTEM_AES_KEY", skillEnvTestAESKey)
	repo := newSkillTestRepo(t)
	ctx := context.Background()
	alice := types.Principal{Type: types.PrincipalWebUser, ID: "u-alice"}
	...
	otherTenant := userEnvRow(alice, "sk-a", "OTHER", "d")
	otherTenant.ID = uuid.NewString()
	otherTenant.TenantID = 8
	require.NoError(t, repo.UpsertUserEnvVar(ctx, otherTenant))

	count, err := repo.CountUserEnvVarsByConfig(ctx, "cfg-1")
	require.NoError(t, err)
	require.Equal(t, int64(4), count)
}

func TestListSkillsReturnsPlatformInstallationsAcrossConfigs(t *testing.T) {
	repo := newSkillTestRepo(t)
	ctx := context.Background()

	require.NoError(t, repo.CreateSkill(ctx, skillRow("sk-a", "cfg-1", "pdf")))
	require.NoError(t, repo.CreateSkill(ctx, skillRow("sk-b", "cfg-2", "search")))
	require.NoError(t, repo.CreateSkill(ctx, skillRow("sk-other", "cfg-3", "pdf")))

	list, err := repo.ListSkills(ctx)
	require.NoError(t, err)
	ids := make([]string, 0, len(list))
	for _, e := range list {
		ids = append(ids, e.ID)
	}
	require.ElementsMatch(t, []string{"sk-a", "sk-b", "sk-other"}, ids)
}

func TestCatalogPreservesDuplicateNamesAsDistinctIdentities(t *testing.T) {
	repo := newSkillTestRepo(t)
	ctx := context.Background()
	...
	got, err := repo.GetCatalogByName(ctx, "pdf")
	require.NoError(t, err)
	...
	list, err := repo.ListSkillsByCatalog(ctx, "cat-a")
	require.NoError(t, err)
	require.Len(t, list, 1)
}
