...
func TestListSkillsReturnsTheConfigsSkills(t *testing.T) {
	...
	skills, err := fx.svc.ListSkills(context.Background(), "cfg-1")
	require.NoError(t, err)
	require.Len(t, skills, 1)
	require.Equal(t, "sk-1", skills[0].ID)
}

func TestGetSkillIsScopedToConfig(t *testing.T) {
	fx := newInstallFixture(t)
	ctx := context.Background()

	skill, err := fx.svc.GetSkill(ctx, "cfg-1", "sk-1")
	require.NoError(t, err)
	require.NotNil(t, skill)

	found, err := fx.svc.GetSkill(ctx, "cfg-2", "sk-1")
	require.NoError(t, err)
	require.Nil(t, found)
}
...
func TestSetSkillEnabledPersistsWithoutTouchingInstallState(t *testing.T) {
	...
	updated, err := fx.svc.SetSkillEnabled(ctx, "cfg-1", "sk-1", false)
	require.NoError(t, err)
	require.NotNil(t, updated)
	require.False(t, updated.Enabled)

	stored, err := fx.skillRepo.GetSkill(ctx, "cfg-1", "sk-1")
	require.NoError(t, err)
	require.False(t, stored.Enabled)
	require.Equal(t, types.SkillStatusInstalling, stored.Status)

	back, err := fx.svc.SetSkillEnabled(ctx, "cfg-1", "sk-1", true)
	require.NoError(t, err)
	require.True(t, back.Enabled)
}

func TestSetSkillEnabledReturnsNilForAnotherConfig(t *testing.T) {
	fx := newInstallFixture(t)

	updated, err := fx.svc.SetSkillEnabled(context.Background(), "cfg-2", "sk-1", false)
	require.NoError(t, err)
	require.Nil(t, updated)

	stored, err := fx.skillRepo.GetSkill(context.Background(), "cfg-1", "sk-1")
	require.NoError(t, err)
	require.True(t, stored.Enabled, "a mismatched config must not hide this skill")
}
...
func seedSkillEnvDeclaration(t *testing.T, fx *installFixture, envs types.SkillEnvVars) {
	t.Helper()
	require.NoError(t, fx.skillRepo.UpdateSkillEnvs(context.Background(), "cfg-1", "sk-1", envs))
}
...
func TestSetSkillEnvValuesWritesOnlyDeclaredNames(t *testing.T) {
	...
	updated, err := fx.svc.SetSkillEnvValues(ctx, "cfg-1", "sk-1", map[string]string{
		"API_TOKEN":  "secret-1",
		"NOT_A_NAME": "ignored",
	})
	require.NoError(t, err)
...
func TestSetSkillEnvValuesEmptyStringClearsTheValueKeepingTheDeclaration(t *testing.T) {
	...
	_, err := fx.svc.SetSkillEnvValues(ctx, "cfg-1", "sk-1", map[string]string{"API_TOKEN": ""})
	require.NoError(t, err)
...
func TestSetSkillEnvValuesRefusesAnOversizedValue(t *testing.T) {
	...
	_, err := fx.svc.SetSkillEnvValues(ctx, "cfg-1", "sk-1", map[string]string{
...
func TestSetSkillEnvValuesReturnsNilForAnUnreachableSkill(t *testing.T) {
	...
	updated, err := fx.svc.SetSkillEnvValues(context.Background(), "cfg-2", "sk-1",
	...
	require.NoError(t, err)
	token, _ := stored.Envs.Get("API_TOKEN")
	require.Empty(t, token.Value, "a mismatched config must not write this value")
}
...
func TestUpdateSkillAdminAppliesEnabledAndValuesAsOneState(t *testing.T) {
	...
	enabled := false
	updated, err := fx.svc.UpdateSkillAdmin(ctx, "cfg-1", "sk-1", SkillAdminUpdate{
		Enabled:   &enabled,
...
func TestUpdateSkillAdminRejectsTheWholeRequestOnAnOversizedValue(t *testing.T) {
	...
	enabled := false
	_, err := fx.svc.UpdateSkillAdmin(ctx, "cfg-1", "sk-1", SkillAdminUpdate{
		Enabled:   &enabled,
...
func TestSubscribeProgressWithoutRedisYieldsNoStream(t *testing.T) {
	...
	events, release, err := fx.svc.SubscribeProgress(context.Background(), "cfg-1", "sk-1")
	require.NoError(t, err)
