...
type switchTenantTenantService struct {
	interfaces.TenantService
}

func (s *switchTenantTenantService) GetTenantByID(_ context.Context, id uint64) (*types.Tenant, error) {
	return &types.Tenant{ID: id, Status: types.TenantStatusActive}, nil
}
...
func newSwitchTenantTestService(repo *switchTenantUserRepo, memberSvc interfaces.TenantMemberService) *userService {
	return &userService{
		...
		tenantService: &switchTenantTenantService{},
...
func TestSwitchTenantPreferenceWriteFailureAbortsSwitch(t *testing.T) {
	...
	resp, err := svc.SwitchTenant(ctx, user, 7, "")
	if err == nil {
...
func TestSwitchTenantPreservesOidcOnlyLogin(t *testing.T) {
	...
	if stored.Preferences.LastActiveTenantID == nil || *stored.Preferences.LastActiveTenantID != 7 {
		t.Fatalf("LastActiveTenantID = %v, want 7", stored.Preferences.LastActiveTenantID)
	}
}
