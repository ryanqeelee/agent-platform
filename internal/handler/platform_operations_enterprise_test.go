...
func (r *recordingEnterpriseOperationsRepository) UpdateForPlatformOperations(
	...
	seatsTotal *int, storageQuota int64,
	...
	return &types.Tenant{
		...
		SeatsTotal: seatsTotal, StorageQuota: storageQuota, StorageUsed: 256,
	}, 2, nil
}
...
func enterprisePatchRouter(repository *recordingEnterpriseOperationsRepository) *gin.Engine {
	...
	handler := NewPlatformOperationsHandler(nil, repository, nil, nil, nil, nil)
...
func TestPlatformOperationsEnterprisePatchPersistsFrozenFields(t *testing.T) {
	...
	body := `{"name":" Acme ","description":"Updated","status":"suspended","seats_total":null,"storage_quota":0}`
	...
	require.JSONEq(t, `{"success":true,"data":{"id":7,"name":"Acme","description":"Updated","status":"suspended","seats_total":null,"seats_used":2,"storage_quota":0,"storage_used":256,"created_at":"0001-01-01T00:00:00Z","updated_at":"0001-01-01T00:00:00Z"}}`, response.Body.String())
}

func TestPlatformOperationsEnterprisePatchRejectsUnknownOrLifecycleStatus(t *testing.T) {
	for name, body := range map[string]string{
		"unknown":      `{"name":"Acme","description":"","status":"active","seats_total":1,"storage_quota":0,"machine_code":"x"}`,
		"provisioning": `{"name":"Acme","description":"","status":"provisioning","seats_total":1,"storage_quota":0}`,
		"negative":     `{"name":"Acme","description":"","status":"active","seats_total":1,"storage_quota":-1}`,
	} {
