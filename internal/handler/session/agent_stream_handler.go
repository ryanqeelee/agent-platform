...
func (h *AgentStreamHandler) handleError(ctx context.Context, evt event.Event) error {
	...
	// EventError is terminal for a turn. The SSE loops close only on a complete
	// marker, so emitting an error alone leaves both the original request and
	// ContinueStream polling forever.
	if err := h.streamManager.AppendEvent(h.ctx, h.sessionID, h.assistantMessageID, interfaces.StreamEvent{
		ID:        fmt.Sprintf("%s-complete", evt.ID),
		Type:      types.ResponseTypeComplete,
		Done:      true,
		Timestamp: time.Now(),
	}); err != nil {
		logger.GetLogger(h.ctx).Error("Append completion after error failed", "error", err)
	}

	return nil
}

func employeeModelRuntimeFailureStatus() types.RoleBoundedOperationalStatus {
