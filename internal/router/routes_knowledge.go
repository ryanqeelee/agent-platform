...
//
// Mutating routes require Knowledge Administrator+ and current write access
// to the owning KB.
func RegisterChunkRoutes(r *gin.RouterGroup, handler *handler.ChunkHandler, g *rbacGuards) {
	...
	chunkRead := chunks.With(apiKeyRetrieve(apiKeyFullAccess()))
	{
		...
		// 删除分块 — Knowledge Administrator+，且对父 KB 有 write 权限
		chunks.DELETE("/:knowledge_id/:id", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("knowledge_id"), handler.DeleteChunk)
		// 删除知识下的所有分块 — Knowledge Administrator+，且对父 KB 有 write 权限
		chunks.DELETE("/:knowledge_id", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("knowledge_id"), handler.DeleteChunksByKnowledgeID)
		// 更新分块信息 — Knowledge Administrator+，且对父 KB 有 write 权限
		chunks.PUT("/:knowledge_id/:id", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("knowledge_id"), handler.UpdateChunk)
		chunks.POST("/:knowledge_id/:id/revert", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("knowledge_id"), handler.RevertChunk)
		// 删除单个生成的问题（通过分块 id） — 与其它 chunk mutation 一致。
		chunks.DELETE("/by-id/:id/questions", g.Admin(), g.KBAccessWriteFromChunkIDParam("id"), handler.DeleteGeneratedQuestion)
		chunks.PUT("/by-id/:id/questions", g.Admin(), g.KBAccessWriteFromChunkIDParam("id"), handler.UpsertGeneratedQuestion)
		chunks.POST("/by-id/:id/questions/regenerate", g.Admin(), g.KBAccessWriteFromChunkIDParam("id"), handler.RegenerateGeneratedQuestions)
	}
}
...
//
// Mutating routes require Knowledge Administrator+ and current write access
// to the owning KB. Cross-resource handlers validate every KB carried in the
// request body.
func RegisterKnowledgeRoutes(r *gin.RouterGroup, handler *handler.KnowledgeHandler, g *rbacGuards) {
	...
	{
		kb.POST("/file", g.Admin(), g.KBAccessWrite("id"), handler.CreateKnowledgeFromFile)
		kb.POST("/url", g.Admin(), g.KBAccessWrite("id"), handler.CreateKnowledgeFromURL)
		kb.POST("/manual", g.Admin(), g.KBAccessWrite("id"), handler.CreateManualKnowledge)
		...
		kb.PUT("/folders", g.Admin(), g.KBAccessWrite("id"), handler.RenameKnowledgeFolder)
	...
	kRead := k.With(apiKeyRetrieve(apiKeyFullAccess()))
	{
		...
		k.DELETE("/:id", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("id"), handler.DeleteKnowledge)
		k.PUT("/:id", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("id"), handler.UpdateKnowledge)
		k.POST("/:id/regenerate-summary", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("id"), handler.RegenerateKnowledgeSummary)
		k.PUT("/manual/:id", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("id"), handler.UpdateManualKnowledge)
		k.POST("/:id/reparse", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("id"), handler.ReparseKnowledge)
		k.POST("/:id/cancel-parse", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("id"), handler.CancelKnowledgeParse)
		...
		kRead.GET("/:id/download", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("id"), handler.DownloadKnowledgeFile)
		kRead.GET("/:id/preview", g.Viewer(), g.KBAccessReadFromKnowledgeIDParam("id"), handler.PreviewKnowledgeFile)
		k.PUT("/image/:id/:chunk_id", g.Admin(), g.KBAccessWriteFromKnowledgeIDParam("id"), handler.UpdateImageInfo)
...
func RegisterFAQRoutes(r *gin.RouterGroup, handler *handler.FAQHandler, g *rbacGuards) {
	...
	// 等价于修改 KB 内容：Viewer+ 可以读，Knowledge Administrator+
	// 可以维护本企业既有 KB 的 FAQ。
	...
	faqRead := faq.With(apiKeyRetrieve(apiKeyFullAccess()))
	{
		...
		faq.POST("/entries", g.Admin(), g.KBAccessWrite("id"), handler.UpsertEntries)
		faq.POST("/entry", g.Admin(), g.KBAccessWrite("id"), handler.CreateEntry)
		faq.PUT("/entries/:entry_id", g.Admin(), g.KBAccessWrite("id"), handler.UpdateEntry)
		faq.POST("/entries/:entry_id/similar-questions", g.Admin(), g.KBAccessWrite("id"), handler.AddSimilarQuestions)
		// Unified batch update API - supports is_enabled, is_recommended, tag_id
		faq.PUT("/entries/fields", g.Admin(), g.KBAccessWrite("id"), handler.UpdateEntryFieldsBatch)
		faq.PUT("/entries/tags", g.Admin(), g.KBAccessWrite("id"), handler.UpdateEntryTagBatch)
		faq.DELETE("/entries", g.Admin(), g.KBAccessWrite("id"), handler.DeleteEntries)
		...
		faq.PUT("/import/last-result/display", g.Admin(), g.KBAccessWrite("id"), handler.UpdateLastImportResultDisplayStatus)
	}
...
func RegisterKnowledgeBaseRoutes(r *gin.RouterGroup, handler *handler.KnowledgeBaseHandler, g *rbacGuards) {
	...
	kbManagement := kb.With(apiKeyManageKnowledgeBases(apiKeyFullAccess()))
	{
		...
		// 更新/删除知识库要求 Admin+；KBAccessWrite 另行校验当前知识授权范围。
		...
		kbManagement.PUT("/:id", g.Admin(), g.KBAccessWrite("id"), handler.UpdateKnowledgeBase)
		kbManagement.DELETE("/:id", g.Admin(), g.KBAccessWrite("id"), handler.DeleteKnowledgeBase)
		// 置顶状态按 (user, kb) 保存。任何具备当前读取权限的成员都可以维护自己的置顶状态。
		// Anyone with
...
// feed. It intentionally stays JWT-only: audit history is a sensitive Admin
...
func RegisterKnowledgeBaseActivityRoutes(r *gin.RouterGroup, auditHandler *handler.AuditLogHandler, g *rbacGuards) {
	...
	r.GET("/knowledge-bases/:id/activity",
		g.Admin(), g.KBAccessRead("id"), auditHandler.ListKnowledgeBaseActivity)
}
...
func RegisterKnowledgeTagRoutes(r *gin.RouterGroup, tagHandler *handler.TagHandler, g *rbacGuards) {
	...
	// 行为，遵循 Knowledge Administrator+ 维护既有企业知识内容的矩阵。
	...
	kbTagsRead := kbTags.With(apiKeyRetrieve(apiKeyFullAccess()))
	{
		...
		kbTags.POST("", g.Admin(), g.KBAccessWrite("id"), tagHandler.CreateTag)
		kbTags.PUT("/:tag_id", g.Admin(), g.KBAccessWrite("id"), tagHandler.UpdateTag)
		kbTags.DELETE("/:tag_id", g.Admin(), g.KBAccessWrite("id"), tagHandler.DeleteTag)
	}
}

// RegisterWikiPageRoutes registers wiki page related routes.
//
// Wiki pages are KB content: reads require current KB access; content and
// maintenance mutations require Knowledge Administrator+ and write access.
func RegisterWikiPageRoutes(r *gin.RouterGroup, wikiHandler *handler.WikiPageHandler, g *rbacGuards) {
	...
	wikiRead := wiki.With(apiKeyRetrieve(apiKeyFullAccess()))
	{
		...
		wiki.POST("/pages", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.CreatePage)
		wiki.PUT("/move-page", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.MovePage)
		wikiRead.GET("/pages/*slug", g.Viewer(), g.KBAccessRead("kb_id"), wikiHandler.GetPage)
		wiki.PUT("/pages/*slug", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.UpdatePage)
		wiki.DELETE("/pages/*slug", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.DeletePage)
		...
		wiki.POST("/revert", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.RevertPage)

		// Folder tree (directory nodes)
		wikiRead.GET("/folders", g.Viewer(), g.KBAccessRead("kb_id"), wikiHandler.ListFolders)
		wiki.POST("/folders", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.CreateFolder)
		wiki.PUT("/folders/:folder_id", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.UpdateFolder)
		wiki.DELETE("/folders/:folder_id", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.DeleteFolder)
		...
		wiki.POST("/rebuild-links", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.RebuildLinks)
		wikiRead.GET("/lint", g.Viewer(), g.KBAccessRead("kb_id"), wikiHandler.Lint)
		wiki.POST("/auto-fix", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.AutoFix)

		// Issues
		wikiRead.GET("/issues", g.Viewer(), g.KBAccessRead("kb_id"), wikiHandler.ListIssues)
		wiki.PUT("/issues/:issue_id/status", g.Admin(), g.KBAccessWrite("kb_id"), wikiHandler.UpdateIssueStatus)
	}
}
