...
var contextCloneAcrossDetach = map[ContextKey]bool{
	...
	// CrossTenantAccessContextKey is the middleware-resolved authorization
	// scope for this principal, after both the server-owned user attribute and
	// deployment gate have been checked. Detached work that continues the same
	// request must see the same scope, just like SystemAdminContextKey; dropping
	// it would make cross-tenant handlers change authorization after cloning.
	CrossTenantAccessContextKey: true,
