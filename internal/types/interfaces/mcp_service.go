...
type MCPServiceRepository interface {
	...
	GetByID(ctx context.Context, id string) (*types.MCPService, error)

	// List retrieves all platform MCP services.
	List(ctx context.Context) ([]*types.MCPService, error)

	// ListEnabled retrieves all enabled platform MCP services.
	ListEnabled(ctx context.Context) ([]*types.MCPService, error)

	// ListByIDs retrieves platform MCP services by multiple IDs.
	ListByIDs(ctx context.Context, ids []string) ([]*types.MCPService, error)
...
type MCPServiceService interface {
	...
	GetMCPServiceByID(ctx context.Context, id string) (*types.MCPService, error)

	// ListMCPServices lists all platform MCP services.
	ListMCPServices(ctx context.Context) ([]*types.MCPService, error)

	// ListMCPServicesByIDs retrieves multiple MCP services by IDs
	ListMCPServicesByIDs(ctx context.Context, ids []string) ([]*types.MCPService, error)
	...
	DeleteMCPService(ctx context.Context, id string) error

	// TestMCPService tests the connection to an MCP service and returns available tools/resources
	TestMCPService(ctx context.Context, id string) (*types.MCPTestResult, error)

	// GetMCPServiceTools retrieves the list of tools from an MCP service
	GetMCPServiceTools(ctx context.Context, id string) ([]*types.MCPTool, error)

	// GetMCPServiceResources retrieves the list of resources from an MCP service
	GetMCPServiceResources(ctx context.Context, id string) ([]*types.MCPResource, error)
	...
	ClearMCPCredential(ctx context.Context, id, field string) error
}
